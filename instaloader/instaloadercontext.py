...
def default_iphone_headers() -> Dict[str, Any]:
    return {'User-Agent': 'Instagram 361.0.0.35.82 (iPad13,8; iOS 18_0; en_US; en-US; scale=2.00; 2048x2732; 452417278) AppleWebKit/420+',
...
class InstaloaderContext:
    ...
    def login(self, user, passwd):
            ...
            # Headers (Updated User-Agent to mimic real iPhone request)
            HEADERS = {
                "User-Agent": (
            ...
            # grabing session id and updating if exists - remove the print when it's working was used for debug purposes
            session_id = session.cookies.get_dict()['sessionid']
    
            if session_id:
                print(f"[+] Login Successful! Your Session ID: {session_id}")
                session.cookies.update({'sessionid': session_id});
            else:
                print("[-] Login failed! Check your credentials or solve the security challenge in the browser.")
